...
template <typename T> struct IsGenericValueImpl<T, typename Void<typename T::EncodingType>::Type, \
	  ...
	  typename T::AllocatorType>, T>::Type {};
...
RAPIDJSON_STATIC_ASSERT(sizeof(uint64) == sizeof(unsigned));
template<typename ValueType>
struct TypeHelper<ValueType, uint64> {
    static bool Is(const ValueType& v) { return v.IsUint(); }
    static uint64 Get(const ValueType& v) { return v.GetUint(); }
    static ValueType& Set(ValueType& v, uint64 data) { return v.SetUint(data); }
    static ValueType& Set(ValueType& v, uint64 data, typename ValueType::AllocatorType&) {
...
public:
                ...
                switch (rhs.GetType()) {
                    case kObjectType: {
                     ...
                     case kArrayType: {
                         ...
                         GenericValue* le = reinterpret_cast<GenericValue*>(allocator.Malloc(count * \
                                sizeof(GenericValue)));
    ...
    */
    explicit GenericValue(Array a) RAPIDJSON_NOEXCEPT : data_(a.value_.data_) {
        a.value_.data_ = Data();
    ...
    */
    explicit GenericValue(Object o) RAPIDJSON_NOEXCEPT : data_(o.value_.data_) {
        o.value_.data_ = Data();
    ...
    */
    ...
    /*! \return (rhs == lhs)
     */
    template <typename T> friend RAPIDJSON_DISABLEIF_RETURN((internal::IsGenericValue<T>), (bool)) operator==(
    ...
    */
    GenericValue& PushBack(GenericValue& value, Allocator& allocator) {
        ...
        if (data_.a.size >= data_.a.capacity)
            Reserve(data_.a.capacity == 0 ? kDefaultArrayCapacity : (data_.a.capacity + (data_.a.capacity \
                   + 1) / 2), allocator);
    ...
    */
    ...
    GenericValue& SetFloat(float f)         { this->~GenericValue(); new (this) GenericValue(static_cast<double>(f));
                                              return *this;
    }
...
private:
    ...
    struct ShortString {
        enum { MaxChars = sizeof(static_cast<Flag*>(0)->payload) / sizeof(Ch),
            MaxSize = MaxChars - 1, LenPos = MaxSize
        };
        Ch str[MaxChars];
...
public:
    ...
    */
    ...
    /*! Creates an empty document which type is Null.
        ...
        \param stackAllocator   Optional allocator for allocating memory for stack.
    */
    explicit GenericDocument(Allocator* allocator = 0, size_t stackCapacity = kDefaultStackCapacity, \
            StackAllocator* stackAllocator = 0) : \
...
public:
    ...
    GenericArray Reserve(SizeType newCapacity, AllocatorType &allocator) const { value_.Reserve(newCapacity, \
            allocator); return *this; }
    GenericArray PushBack(ValueType& value, AllocatorType& allocator) const { value_.PushBack(value, \
            allocator); return *this; }
#if RAPIDJSON_HAS_CXX11_RVALUE_REFS
...
private:
    GenericArray();
    explicit GenericArray(ValueType& value) : value_(value) {}
    ValueType& value_;
};
...
public:
    ...
    typedef GenericMemberIterator<Const, typename ValueT::EncodingType,
            typename ValueT::AllocatorType> MemberIterator;
    ...
    template <typename SourceAllocator> ValueType& operator[](const GenericValue<EncodingType, \
            SourceAllocator>& name) const {
        return value_[name];
    }
#if RAPIDJSON_HAS_STDSTRING
    ...
    GenericObject MemberReserve(SizeType newCapacity, AllocatorType &allocator) const {
        value_.MemberReserve(newCapacity, allocator); return *this;
    }
    ...
    template <typename SourceAllocator> MemberIterator FindMember(const GenericValue<EncodingType,
        SourceAllocator>& name) const {
...
#endif
    template <typename T> RAPIDJSON_DISABLEIF_RETURN((internal::OrExpr<internal::IsPointer<T>,
             internal::IsGenericValue<T> >), (ValueType&)) AddMember(ValueType& name, T value, \
...
#endif
    template <typename SourceAllocator> bool RemoveMember(const GenericValue<EncodingType, \
            SourceAllocator>& name) const {
...
#endif
    template <typename SourceAllocator> bool EraseMember(const GenericValue<EncodingType, \
            SourceAllocator>& name) const {
...
private:
    GenericObject();
    explicit GenericObject(ValueType& value) : value_(value) {}
    ValueType& value_;
};
