...
public:
    ...
    explicit Array(std::vector<std::unique_ptr<Element>>&& elements);
...
public:
//! Initialize from element vector (move semantic)
        explicit Map(std::vector<std::unique_ptr<Element>>&& elements);
        explicit Map(std::map<std::string, std::unique_ptr<Element>>&& elements);
        std::unique_ptr<Element> copy() const;
        void toJSON(std::ostream& stream) const;
        uint32_t getSizeInBytes() const;
        std::vector<std::unique_ptr<Element>>* getElementsVector();
