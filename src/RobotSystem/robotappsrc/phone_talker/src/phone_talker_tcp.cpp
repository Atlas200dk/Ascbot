...
bool CPhoneTalkerTcp::talk2Tcp(std::string frame, std::string content) {
  ...
  switch (/*socket*/phoneSocket->getStatus()) {
    case netLink::Socket::Status::READY:
      std::cout << "Talk to: " << /*socket*/phoneSocket->hostRemote << ":" \
            << /*socket*/phoneSocket->portRemote << std::endl;
    ...
    case netLink::Socket::Status::NOT_CONNECTED:
      std::cout << "Lost connection of " << /*socket*/phoneSocket->hostRemote << ":" \
              << /*socket*/phoneSocket->portRemote << std::endl;
      // todo ?
    ...
    break;
    default:
      std::cout << "Status of " << /*socket*/phoneSocket->hostRemote << ":" << /*socket*/\
             phoneSocket->portRemote << " is on the status -- " << /*socket*/phoneSocket->getStatus() << \
                     ". It's not ready. " << std::endl;
    break;
  }
  return ret;
}
...
void CPhoneTalkerTcp::run() {
  ...
  socketManager.onConnectRequest = [this](netLink::SocketManager* manager, \
           std::shared_ptr<netLink::Socket> serverSocket, std::shared_ptr<netLink::Socket> clientSocket) {
  ...
  socketManager.onStatusChange = [this](netLink::SocketManager* manager, std::shared_ptr<netLink::Socket> \
          socket, netLink::Socket::Status prev) {
    ...
    switch (socket->getStatus()) {
      ...
      break;
      default:
        std::cout << "Status of " << socket->hostRemote << ":" << socket->portRemote << " changed from " \
               << prev << " to " << socket->getStatus() << std::endl;
      break;
    }
  };

  // Define a callback, fired when a socket receives data
  socketManager.onReceiveMsgPack = [this](netLink::SocketManager* manager, std::shared_ptr<netLink::Socket> socket, \
...
//        }
    ...
    // std::cout << "Enter a IP-Adress to connect to a sever or '*'
    // to start a server (Port: " << nPort << "):" << std::endl;
