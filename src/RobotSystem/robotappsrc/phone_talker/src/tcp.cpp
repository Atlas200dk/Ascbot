...
int main(int argc, char** argv) {
    ...
    socketManager.onConnectRequest = [](netLink::SocketManager* manager, \
    ...
    socketManager.onStatusChange = [](netLink::SocketManager* manager, std::shared_ptr<netLink::Socket> socket, \
        ...
        switch (socket->getStatus()) {
            ...
            break;
            default:
                std::cout << "Status of " << socket->hostRemote << ":" << socket->portRemote \
                             << " changed from " << prev << " to " << socket->getStatus() << std::endl;
            break;
        }
    };
    ...
    int nPort = 3823;
    std::cout << "Enter a IP-Adress to connect to a sever or '*' to start a server (Port: " \
            << nPort << "):" << std::endl;
    while (1) {
        try {
