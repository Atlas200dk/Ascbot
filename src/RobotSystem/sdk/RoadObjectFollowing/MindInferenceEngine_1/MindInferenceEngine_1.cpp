...
*/
void RoadObjectInferenceEngine::ClearOutData() {
...
*/
HIAI_StatusT RoadObjectInferenceEngine::Init(
...
*/
void RoadObjectInferenceEngine::HandleExceptions(std::string error_msg) {
...
*/
HIAI_StatusT RoadObjectInferenceEngine::SendSentinelImage() {
    ...
    do {
        ...
        if (HIAI_OK != hiai_ret) {
            ...
            HIAI_ENGINE_LOG(HIAI_IDE_INFO,
                            "[MindInferenceEngine_1] SendData return value[%d] not "
                            "OK, sleep 200ms",
                            hiai_ret);
...
* @return: HIAI_StatusT
*/
HIAI_StatusT RoadObjectInferenceEngine::PrepareInputBuffer(uint8_t* input_buffer,
    ...
    for (int j = 0; j < batch_size; j++) {
        if (batch_begin + j < image_number) {
            ...
            if (memset_s(input_buffer + j * image_size, image_size, static_cast<char>(0),
                         image_size)) {
                HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                                "[MindInferenceEngine_1] ERROR, batch padding for "
                                "image data failed");
                return HIAI_ERROR;
            }
        }
...
* @return: HIAI_StatusT
*/
HIAI_StatusT RoadObjectInferenceEngine::PrepareInforInput(
...
* @return: HIAI_StatusT
*/
HIAI_StatusT RoadObjectInferenceEngine::Predict() {
...
* @return: HIAI_StatusT
*/
HIAI_StatusT RoadObjectInferenceEngine::SetOutputStruct(const int batch_begin) {
...
*/
void RoadObjectInferenceEngine::SendResult() {
    HIAI_StatusT hiai_ret = HIAI_OK;
    do {
        hiai_ret = SendData(0, "EngineTransT", std::static_pointer_cast<void>(tran_data));
...
* @in: index of the begin of this batch
*/
void RoadObjectInferenceEngine::HandleModelBatchFailure(const int batch_begin,
                                                        const int image_number) {
...
*/
HIAI_IMPL_ENGINE_PROCESS("RoadObjectInferenceEngine", RoadObjectInferenceEngine, INPUT_SIZE) {
    ...
    if (info_number != image_number) {
        HandleExceptions(
            "[MindInferenceEngine_1] ERROR the number of image data and "
            "information data doesn't match!");
    }
    ...
    for (int i = 0; i < image_number; i += batch_size) {
        ...
        // init the output buffer for one dataset batch(might be multiple model
        // batches)
        if (tran_data->output_data_vec.empty()) {
            tran_data->size = output_data_vec.size();
            HIAI_ENGINE_LOG(HIAI_IDE_INFO,
                            "[MindInferenceEngine_1] alloc memory for dataset batch, "
                            "number of outputs of the network: %d",
