...
RoadObjectPreProcess::~RoadObjectPreProcess() {
...
HIAI_StatusT RoadObjectPreProcess::Init(const hiai::AIConfig& config,
    ...
    // get config from RoadObjectPreProcess Property setting of user.
    std::stringstream ss;
...
//  3. DVPP_CTL_VPC_PROC
int RoadObjectPreProcess::__handle_jpeg(const ImageData<u_int8_t>& img) {
    if (NULL == m_pidvppapi_) {
    ...
    jpegdInData.isYUV420Need = false;  // (*m_dvpp_config_).yuv420_need;true:output
                                       // yuv420 data, otherwize:raw format.
    jpegdInData.isVBeforeU = true;     // currently, only support V before U,
                                       // reserved
    ...
    if (0 != DvppCtl(m_pidvppapi_, DVPP_CTL_JPEGD_PROC,
                     &dvppApiCtlMsg)) {  // if this single jpeg pic is processed
                                         // with error, return directly, and then
                                         // process next pic if there any.
    ...
    jpegdOutData.cbFree();  // release memory from caller.
    if (HIAI_OK != ret) {   // if vpc process with error, return directly, and then
                            // process next pic if there any.
...
int RoadObjectPreProcess::__update_crop_para(const Rectangle<Point2D>& rect) {
...
bool RoadObjectPreProcess::__send_data() {
    if (NULL == m_dvpp_out_) {
...
void RoadObjectPreProcess::__clear_data() {
    m_dvpp_in_ = NULL;
    m_dvpp_out_ = NULL;
}
...
//  2. DVPP_CTL_TOOL_CASE_GET_RESIZE_PARAM
//  3. DVPP_CTL_VPC_PROC
int RoadObjectPreProcess::__handle_png(const ImageData<u_int8_t>& img) {
    if (NULL == m_pidvppapi_) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[ImagePreProcess_1] m_pidvppapi_ is null");
        return HIAI_ERROR;
    }
    ...
    if (0 != DvppCtl(m_pidvppapi_, DVPP_CTL_PNGD_PROC,
                     &dvppApiCtlMsg)) {  // if this single jpeg pic is processed
                                         // with error, return directly, and
                                         // process next pic
...
bool RoadObjectPreProcess::__need_crop() {
    bool crop = true;
...
bool RoadObjectPreProcess::__process_crop(VpcUserCropConfigure& area, const int& width,
    ...
    if (__need_crop()) {
        ...
        // amend input offset to avoid the processed range to exceed real range of
        // image
        ...
        HIAI_ENGINE_LOG(HIAI_IDE_INFO,
                        "[ImagePreProcess_1] leftOffset: %u, rightOffset: %u, "
                        "upOffset: %u, downOffset: %u",
                        leftOffset, rightOffset, upOffset, downOffset);
    }

    // check param validity(range---max:4096*4096, min:16*16), leftOffset and
    // upOffset cannot larger than real width and height
    ...
    // restriction: leftOffset and upOffset of inputputArea must be even,
    // rightOffset and downOffset of inputputArea must be odd.
...
int RoadObjectPreProcess::__handle_vpc(const ImageData<u_int8_t>& img, int64_t timestamp) {
    ...
    if (0 != (img.width % VPC_OUT_WIDTH_STRIDE) || 0 != (img.height % VPC_OUT_HIGH_STRIDE)) {
        ...
        // std::cout << "__handle_vpc memcpy before, align_image_len:" <<
        // align_image_len << std::endl;
    ...
    if (HIAI_OK != ret) {  // if vpc process with error, return directly, and then
                           // process next.
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                        "[ImagePreProcess_1] call DVPP_CTL_VPC_PROC process faild");
...
int RoadObjectPreProcess::__handle_vpc_with_param(const unsigned char* buffer, const int& width,
    ...
    HIAI_ENGINE_LOG(
        "[ImagePreProcess_1] real_width: %d, real_height: %d, width: %d, height: "
        "%d",
    ...
    if (type == FILE_TYPE_PIC_JPEG) {
        switch (format) {  // format is responding to color sapce after decoder,
                           // which is needed to match the input color space of vpc
    ...
    if (0 == resize_width || 0 == resize_height) {
        HIAI_ENGINE_LOG(HIAI_IDE_INFO,
                        "[ImagePreProcess_1] user donnot need resize, resize "
    ...
    if (resize_width > DVPP_SUPPORT_MAX_WIDTH || resize_width < DVPP_SUPPORT_MIN_WIDTH ||
        ...
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                        "[ImagePreProcess_1]resize range error, resize_width:%u, "
                        "resize_height:%u",
...
int RoadObjectPreProcess::storePreprocessImage(const u_int8_t* out_buffer, const uint32_t& size,
    ...
    // important:after dvpp, the output data is that width is aligned with 128,
    // and height is aligned with 16.
    ...
    HIAI_ENGINE_LOG(
        "preprocess info, resize_width:%u, resize_height:%u, "
        "preprocess_width:%u, preprocess_height:%u, frameID:%u, order:%u",
    ...
    HIAI_ENGINE_LOG(HIAI_IDE_INFO,
                    "[ImagePreProcess_1]storePreprocessImage, storage success!, "
                    "sendSize:%u, size:%u",
                    sendSize, size);
    return HIAI_OK;
}

int RoadObjectPreProcess::__handle_dvpp() {
...
/**
* @ingroup hiaiengine
* @brief HIAI_DEFINE_PROCESS: realize RoadObjectPreProcess
* @[in]: define a input and output,
*        and register Engine named RoadObjectPreProcess
*/
HIAI_IMPL_ENGINE_PROCESS("RoadObjectPreProcess", RoadObjectPreProcess, INPUT_SIZE) {
    ...
    if (isSentinelImage(m_dvpp_in_)) {
        ...
        do {
            ...
            if (HIAI_OK != hiai_ret) {
                ...
                HIAI_ENGINE_LOG(
                    HIAI_IDE_INFO,
                    "[ImagePreProcess_1] SendData return value[%d] not OK, sleep 200ms", hiai_ret);
                usleep(SEND_DATA_INTERVAL_MS);
            }
    ...
    // 2.preprocess before vpc(DVPP_CTL_JPEGD_PROC, DVPP_CTL_PNGD_PROC, or
    // something else)
    // 3.calculate width, height, and so on, according to the user config of crop
    // and resize, and DVPP_CTL_TOOL_CASE_GET_RESIZE_PARAM
    // 4.DVPP_CTL_VPC_PROC after create dvppapi_ctl_msg para
    // 5.dump yuv data to project dir out after dvpp, according to dump_value
    // config(from device to IDE with IDE-deamon-client&IDE-deamon-hiai process)
...
ERROR:
    // send null to next node to avoid blocking when to encounter abnomal
    // situation.
