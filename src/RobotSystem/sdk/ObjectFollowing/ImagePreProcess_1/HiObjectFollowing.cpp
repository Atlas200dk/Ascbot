...
static const uint32_t GRAPH_ID = 1984830906;
#define SRC_ENGINE_ID 280
#define DEST_ENGINE_ID 483

namespace hi {

class HiObjectImpl : public hiai::DataRecvInterface {
public:
    HiObjectImpl();
    explicit HiObjectImpl(const std::string& filename);
    virtual ~HiObjectImpl();
    /**
    ...
    */
    ...
    void setCallbackFunction(of_callback_t func);
...
HiObjectImpl::HiObjectImpl() {
}

HiObjectImpl::HiObjectImpl(const std::string& filename) : file_name_(filename) {
}
...
HIAI_StatusT HiObjectImpl::HIAI_InitAndStartGraph(std::string graph_config) {
    ...
    for (int i = 0; i < 1; i++) {
        ...
        graph->SetDataRecvFunctor(target_port_config, std::shared_ptr<HiObjectImpl>(this));
    }
    return HIAI_OK;
}

/**
* @ingroup HiObjectImpl
* @brief RecvData RecvData
* @param [in]
*/
HIAI_StatusT HiObjectImpl::RecvData(const std::shared_ptr<void>& message) {
    ...
    for (uint32_t i = 0; i < data->objects.size(); i++) {
        ...
        object.x = one.data[0];
        object.y = one.data[1];
        object.width = one.data[2];
        object.height = one.data[3];
...
bool HiObjectImpl::init(const std::string& config_path) {
    LOGI("init");
...
void HiObjectImpl::setCallbackFunction(of_callback_t func) {
    callback_func = func;
}

bool HiObjectImpl::predict(const HIImgData& frame) {
    if (!isInit()) {
...
HIAI_StatusT HiObjectImpl::makeImageInfo(NewImageParaT* imgData, const HIImgData& input) {
    int32_t img_size = 0;
...
bool HiObjectImpl::isInit() const {
    return isInit_;
}

void HiObjectImpl::setConfidence(float value) {
    conf_threShold = value;
}

HiObjectFollowing::HiObjectFollowing() {
    impl = new HiObjectImpl();
}

HiObjectFollowing::~HiObjectFollowing() {
    if (nullptr != impl) {
        delete reinterpret_cast<HiObjectImpl*>(impl);
        impl = nullptr;
    }
}

bool HiObjectFollowing::init(const std::string& config_path) {
    if (nullptr != impl) {
        return (reinterpret_cast<HiObjectImpl*>(impl))->init(config_path);
    } else {
        LOGE("HiObjectFollowing::init can not init!");
        return false;
    }
}

bool HiObjectFollowing::deInit() {
    if (nullptr != impl && isInit()) {
        return (reinterpret_cast<HiObjectImpl*>(impl))->deInit();
    } else {
        LOGE("HiObjectFollowing::deInit can not init!");
        return false;
    }
}

void HiObjectFollowing::setCallbackFunction(of_callback_t func) {
    if (nullptr != impl && isInit()) {
        return (reinterpret_cast<HiObjectImpl*>(impl))->setCallbackFunction(func);
    } else {
        LOGE("HiObjectFollowing::setCallbackFunction can not init!");
        return;
    }
}

bool HiObjectFollowing::predict(const HIImgData& frame) {
    if (nullptr != impl && isInit()) {
        return (reinterpret_cast<HiObjectImpl*>(impl))->predict(frame);
    } else {
        LOGE("HiObjectFollowing::predict can not init!");
        return false;
    }
}

bool HiObjectFollowing::isInit() const {
    if (nullptr != impl) {
        return (reinterpret_cast<HiObjectImpl*>(impl))->isInit();
    } else {
        LOGE("HiObjectFollowing::isInit can not init!");
        return false;
    }
}

void HiObjectFollowing::setConfidence(float value) {
    if (nullptr != impl) {
        return (reinterpret_cast<HiObjectImpl*>(impl))->setConfidence(value);
    } else {
        LOGE("HiObjectFollowing::setConfidence can not init!");
    }
}
