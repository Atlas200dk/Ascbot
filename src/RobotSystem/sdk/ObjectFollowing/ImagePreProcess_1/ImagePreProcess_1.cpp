...
ObjectPreProcess::~ObjectPreProcess() {
...
HIAI_StatusT ObjectPreProcess::Init(const hiai::AIConfig& config,
    ...
    // get config from ObjectPreProcess Property setting of user.
    std::stringstream ss;
...
//  3. DVPP_CTL_VPC_PROC
int ObjectPreProcess::__handle_jpeg(const ImageData<u_int8_t>& img) {
    if (NULL == m_pidvppapi_) {
...
int ObjectPreProcess::__update_crop_para(const Rectangle<Point2D>& rect) {
...
bool ObjectPreProcess::__send_data() {
    if (NULL == m_dvpp_out_) {
...
void ObjectPreProcess::__clear_data() {
    m_dvpp_in_ = NULL;
    m_dvpp_out_ = NULL;
}
...
//  2. DVPP_CTL_TOOL_CASE_GET_RESIZE_PARAM
//  3. DVPP_CTL_VPC_PROC
int ObjectPreProcess::__handle_png(const ImageData<u_int8_t>& img) {
    if (NULL == m_pidvppapi_) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[ImagePreProcess_1] m_pidvppapi_ is null");
        return HIAI_ERROR;
    }
...
bool ObjectPreProcess::__need_crop() {
    bool crop = true;
...
bool ObjectPreProcess::__process_crop(VpcUserCropConfigure& area, const int& width,
...
int ObjectPreProcess::__handle_vpc(const ImageData<u_int8_t>& img, int64_t timestamp) {
...
int ObjectPreProcess::__handle_vpc_with_param(const unsigned char* buffer, const int& width,
    ...
    if (0 == resize_width || 0 == resize_height) {
        HIAI_ENGINE_LOG(HIAI_IDE_INFO,
                        "[ImagePreProcess_1] user donnot need resize, resize width/height use "
                        "real size of pic");
...
int ObjectPreProcess::storePreprocessImage(const u_int8_t* out_buffer, const uint32_t& size,
    ...
    HIAI_ENGINE_LOG(
        HIAI_IDE_INFO,
        "[ImagePreProcess_1]storePreprocessImage, storage success!, sendSize:%u, size:%u",
        sendSize, size);
    return HIAI_OK;
}

int ObjectPreProcess::__handle_dvpp() {
...
/**
* @ingroup hiaiengine
* @brief HIAI_DEFINE_PROCESS: realize ObjectPreProcess
* @[in]: define a input and output,
*        and register Engine named ObjectPreProcess
*/
HIAI_IMPL_ENGINE_PROCESS("ObjectPreProcess", ObjectPreProcess, INPUT_SIZE) {
