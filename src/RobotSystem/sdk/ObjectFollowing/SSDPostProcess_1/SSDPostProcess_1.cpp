...
* @[in]: value, the port setting string, format is 'attribute,confidence,port',e.g.:
* "14,50,0;7,70,1;"
*/
void ObjectPostProcess::SetOutputFilter(const std::string value) {
    ...
    while (std::getline(output_config_ss, cls_config, ';')) {
        ...
        while (cls_config_ss >> cls) {
            ...
            HIAI_ENGINE_LOG(HIAI_IDE_INFO,
                            "[SSDPostProcess_1] test output settings: port: %d, conf: %f, cls: %d",
...
*/
void ObjectPostProcess::WriteResultFile(ImageInfor img_infor,
...
*/
void ObjectPostProcess::InitOutputs(std::shared_ptr<EngineTransT> tran) {
    outputs.clear();
...
* @[in]: out_bbox: struct of the detected content, including bbox coordinates, predicted classes
* and confidence
* @[in]: output_data_vec: output of inference engine for one batch
*/
void ObjectPostProcess::GetRealInputs(OutputT& out_num, OutputT& out_bbox,
...
* init
*/
HIAI_StatusT ObjectPostProcess::Init(const hiai::AIConfig& config,
...
*/
void ObjectPostProcess::ResolveBBox(float* ptr, std::vector<float>& attribute_number,
...
void ObjectPostProcess::ResolveBBox(float* ptr, ObjectT& object, const CropInfo& crop) {
    if (ptr == nullptr) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[SSDPostProcess_1] ptr is nullptr");
        return;
    }
...
void ObjectPostProcess::CaffeResolveBBox(float* ptr, int ptrSize, const ImageInfor img_infor,
...
*/
void ObjectPostProcess::BuildBatch() {
    if (outputs.count(-1) != 0) {
...
*/
void ObjectPostProcess::SendDataToPorts() {
    for (int i = 0; i < postprocess_config_->port_list.size(); i++) {
...
void ObjectPostProcess::SendDataToPorts(std::shared_ptr<ObjectResultT>& data) {
    for (int i = 0; i < postprocess_config_->port_list.size(); i++) {
        if (postprocess_config_->port_list[i] == false) {
            HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[SSDPostProcess_1] no filter! port: %d", i);
...
*/
void ObjectPostProcess::HandleExceptions(int output_size) {
    if (output_size != 1) {
...
*/
void ObjectPostProcess::CheckStatus(HIAI_StatusT ret_status, std::string error_msg) {
...
*/
HIAI_StatusT ObjectPostProcess::SendSentinelImage() {
...
* @in: tran: output struct received from previous engine
*/
HIAI_StatusT ObjectPostProcess::caffeSSDProcess(std::vector<OutputT>& output_data_vec,
...
/**
* @brief: reset the cropinfo to make sure the output bbox coordinates are always relative to the
* original image
* @in: crop: the crop info got from preprocess, which will be reseted for later transformation
* process
* @in: img_infor: the image information of the current image
*/
void ObjectPostProcess::SetTransformationInfor(CropInfo& crop, const ImageInfor img_infor) {
...
HIAI_IMPL_ENGINE_PROCESS("ObjectPostProcess", ObjectPostProcess, INPUT_SIZE) {
    ...
    for (int ind = 0; ind < tran->b_info.batch_size; ind++) {
        ...
        img_infor.width = 1280;
        img_infor.height = 720;
        auto crop = tran->v_img[ind].crop_info;
        // std::cout << "crop width:" << crop.crop_width << ",height:" << crop.crop_width <<
        // std::endl;
        // std::cout << "crop point x:" << crop.point_x << ",y:" << crop.point_y << std::endl;
        ...
        // std::cout << "batch_result:" << batch_result << ",buffer_size:" << buffer_size <<
        // ",ind:" << ind << std::endl;
        ...
        // std::cout << "bbox_buffer_size:" << bbox_buffer_size << ", bbox_count:" << *bbox_count
        // << ",sizePerClassify:" << sizePerClassify << std::endl;
        ...
        //     rect.anchor_lt.y << "," << rect.anchor_rb.x << "," << rect.anchor_rb.y << "]" << ",
        //     conf:" << conf << std::endl;
        // }
        // add the current result into batch result
        // HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[SSDPostProcess_1] frameid: %d, name: %s",
        // frame_ID[ind], img_infor.tfilename.c_str());
        // BuildBatch();
        // write output file
        // WriteResultFile(img_infor, attribute_number, confidence, bbox);
    }
    // std::cout << "OUTPUT_SIZE:" << OUTPUT_SIZE << std::endl;
    // std::cout << "send result to port! size:" << output_result->objects.size() << std::endl;
    delete[] batch_result;
    uint8_t port = 0;
