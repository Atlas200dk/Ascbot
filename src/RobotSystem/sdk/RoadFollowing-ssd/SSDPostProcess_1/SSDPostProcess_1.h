...
class RFPostProcess : public Engine {
private:
...
private:
    ...
    std::shared_ptr<hiai::BatchDetectedObjectPara<hiai::Rectangle<hiai::Point2D>, float>> batchDetected;
    std::unordered_map<int, hiai::ObjectLocation<hiai::Rectangle<hiai::Point2D>, float>> object_locs;
    std::unordered_map<
        int, std::shared_ptr<hiai::BatchDetectedObjectPara<hiai::Rectangle<hiai::Point2D>, float>>>
        outputs;
    ...
    */
    void ResolveBBox(float* ptr, std::vector<float>& attribute_number,
                     std::vector<float>& confidence, std::vector<hiai::Rectangle<hiai::Point2D>>& bbox,
                     const CropInfo& crop);
    ...
    */
    ...
    void SendDataToPorts(std::shared_ptr<RFResultT>& data);
