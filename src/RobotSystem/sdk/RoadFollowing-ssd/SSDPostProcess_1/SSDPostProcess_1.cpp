...
*/
void RFPostProcess::SetOutputFilter(const std::string value) {
...
*/
void RFPostProcess::WriteResultFile(ImageInfor img_infor,
                                    const std::vector<float>& attribute_number,
                                    const std::vector<float>& confidence,
                                    const std::vector<hiai::Rectangle<hiai::Point2D>>& bbox) {
...
*/
void RFPostProcess::InitOutputs(std::shared_ptr<EngineTransT> tran) {
    outputs.clear();
    for (int j = 0; j < postprocess_config_->port_list.size(); j++) {
        if (postprocess_config_->port_list[j] == true) {
            outputs[j] =
                std::make_shared<hiai::BatchDetectedObjectPara<hiai::Rectangle<hiai::Point2D>, float>>();
...
*/
void RFPostProcess::GetRealInputs(OutputT& out_num, OutputT& out_bbox,
...
*/
void RFPostProcess::ResolveBBox(float* ptr, std::vector<float>& attribute_number,
                                std::vector<float>& confidence,
                                std::vector<hiai::Rectangle<hiai::Point2D>>& bbox, const CropInfo& crop) {
    if (ptr == nullptr) {
    ...
    float attr = ptr[1];
    float score = ptr[2];
    hiai::Rectangle<hiai::Point2D> rect;
...
void RFPostProcess::ResolveBBox(float* ptr, ObjectT& object, const CropInfo& crop) {
    if (ptr == nullptr) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[SSDPostProcess_1] ptr is nullptr");
        return;
    }
    ...
    object.raw_data[0] = ptr[3];
    object.raw_data[1] = ptr[4];
    object.raw_data[2] = ptr[5];
    object.raw_data[3] = ptr[6];
    return;
}
...
void RFPostProcess::CaffeResolveBBox(float* ptr, int ptrSize, const ImageInfor img_infor,
                                     std::vector<float>& attribute_number,
                                     std::vector<float>& confidence,
                                     std::vector<hiai::Rectangle<hiai::Point2D>>& bbox,
                                     hiai::ObjectLocation<hiai::Rectangle<hiai::Point2D>, float>& object_loc) {
    ...
    float attr = ptr[1];
    float score = ptr[2];
    hiai::Rectangle<hiai::Point2D> rect = {{0, 0}};
...
*/
void RFPostProcess::SendDataToPorts() {
    for (int i = 0; i < postprocess_config_->port_list.size(); i++) {
...
void RFPostProcess::SendDataToPorts(std::shared_ptr<RFResultT>& data) {
    for (int i = 0; i < postprocess_config_->port_list.size(); i++) {
        if (postprocess_config_->port_list[i] == false) {
            HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[SSDPostProcess_1] no filter! port: %d", i);
            HIAI_StatusT ret = SendData(i, "RFResultT", std::static_pointer_cast<void>(data));
            CheckStatus(ret, "[SSDPostProcess_1] failed to send");
        } else {
            HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[SSDPostProcess_1] filter! port: %d", i);
            HIAI_StatusT send_ret = SendData(i, "RFResultT", std::static_pointer_cast<void>(data));
            CheckStatus(send_ret, "[SSDPostProcess_1] failed to send");
        }
    }
}
...
* recall functor)
*/
void RFPostProcess::HandleExceptions(int output_size) {
    if (output_size != 1) {
...
*/
void RFPostProcess::CheckStatus(HIAI_StatusT ret_status, std::string error_msg) {
...
* @in: tran: output struct received from previous engine
*/
HIAI_StatusT RFPostProcess::caffeSSDProcess(std::vector<OutputT>& output_data_vec,
...
*/
void RFPostProcess::SetTransformationInfor(CropInfo& crop, const ImageInfor img_infor) {
...
HIAI_IMPL_ENGINE_PROCESS("RFPostProcess", RFPostProcess, INPUT_SIZE) {
    ...
    std::vector<float> confidence;
    std::vector<hiai::Rectangle<hiai::Point2D>> bbox;
    std::shared_ptr<RFResultT> output_result(new RFResultT());
    ...
    for (int ind = 0; ind < tran->b_info.batch_size; ind++) {
        ...
        // std::cout << "batch_result:" << batch_result
        //           << ",buffer_size:" << buffer_size << ",ind:" << ind << std::endl;
        ...
        // std::cout << "bbox_buffer_size:" << bbox_buffer_size
    ...
    HIAI_StatusT send_ret =
        SendData(port, "RFResultT", std::static_pointer_cast<void>(output_result));
    CheckStatus(send_ret, "[SSDPostProcess_1] failed to send");
