...
CollisionPreProcess::~CollisionPreProcess() {
    if (NULL != m_pidvppapi_) {
        DestroyDvppApi(m_pidvppapi_);
        m_pidvppapi_ = NULL;
    }
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[CollisionPreProcess]CollisionPreProcess Engine Destory");
}

HIAI_StatusT CollisionPreProcess::Init(const hiai::AIConfig& config,
    ...
    if (NULL == m_dvpp_config_) {
        ...
        if (NULL == m_dvpp_config_ || NULL == m_dvpp_config_.get()) {
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                            "[CollisionPreProcess] call m_dvpp_config_ make_shared failed");
            return HIAI_ERROR;
        }
    }

    // get config from CollisionPreProcess Property setting of user.
    std::stringstream ss;
    ...
    if (DVPP_SUCCESS != CreateDvppApi(m_pidvppapi_)) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionPreProcess]Create DVPP m_pidvppapi_ fail");
        return HIAI_ERROR;
    }
...
//  3. DVPP_CTL_VPC_PROC
int CollisionPreProcess::__handle_jpeg(const ImageData<u_int8_t>& img) {
    if (NULL == m_pidvppapi_) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionPreProcess] m_pidvppapi_ is null!\n");
        return HIAI_ERROR;
    }
    ...
    if (0 != DvppCtl(m_pidvppapi_, DVPP_CTL_JPEGD_PROC,
        ...
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionPreProcess] JPEGDERROR, FrameID:%u",
                        m_imageFrameID);
    ...
    if (HIAI_OK != ret) {  // if vpc process with error, return directly, and then process next pic
                           // if there any.
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionPreProcess]VPCERROR, FrameID:%u",
                        m_imageFrameID);
        return HIAI_ERROR;
    }
    return HIAI_OK;
...
int CollisionPreProcess::__update_crop_para(const Rectangle<Point2D>& rect) {
    ...
    HIAI_ENGINE_LOG("[CollisionPreProcess]: %d, %d, %d, %d", (*m_dvpp_config_).point_x,
    ...
    if ((*m_dvpp_config_).point_x < 0 || (*m_dvpp_config_).point_x % 2 != 0 ||
        ...
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionPreProcess]Some crop parms are invalid!");
        return HIAI_ERROR;
    }
    return HIAI_OK;
}

bool CollisionPreProcess::__send_data() {
    if (NULL == m_dvpp_out_) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionPreProcess]Nothing to send!");
        return false;
    }
    ...
    if (m_dvpp_out_->v_img.size() > 0) {
        ...
        do {
            ...
            if (HIAI_QUEUE_FULL == hiai_ret) {
                HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[CollisionPreProcess] queue full, sleep 200ms");
        ...
        if (HIAI_OK != hiai_ret) {
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                            "[CollisionPreProcess] SendData failed! error code: %d", hiai_ret);
            return false;
        }
    }
    return true;
...
void CollisionPreProcess::__clear_data() {
    m_dvpp_in_ = NULL;
    m_dvpp_out_ = NULL;
}
...
//  2. DVPP_CTL_TOOL_CASE_GET_RESIZE_PARAM
//  3. DVPP_CTL_VPC_PROC
int CollisionPreProcess::__handle_png(const ImageData<u_int8_t>& img) {
    if (NULL == m_pidvppapi_) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionPreProcess] m_pidvppapi_ is null");
        return HIAI_ERROR;
    }
    ...
    if (0 != DvppCtl(m_pidvppapi_, DVPP_CTL_PNGD_PROC,
        ...
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionPreProcess]PNGDERROR, FrameID:%u",
                        m_imageFrameID);
    ...
    int ret =
        __handle_vpc_with_param((unsigned char*)outputPngData.outputData, outputPngData.widthAlign,
                                outputPngData.highAlign, outputPngData.outputSize, img,
                                FILE_TYPE_PIC_PNG, outputPngData.format, 0);
    HIAI_DVPP_DFree(outputPngData.address);  // release memory from caller.
    outputPngData.address = NULL;
...
bool CollisionPreProcess::__need_crop() {
    bool crop = true;
...
bool CollisionPreProcess::__process_crop(VpcUserCropConfigure& area, const int& width,
    ...
    // user crop
    if (__need_crop()) {
        HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[CollisionPreProcess] User crop, System crop");
        ...
        HIAI_ENGINE_LOG(
            HIAI_IDE_INFO,
            "[CollisionPreProcess] leftOffset: %u, rightOffset: %u, upOffset: %u, downOffset: %u",
            leftOffset, rightOffset, upOffset, downOffset);
    }

    // check param validity(range---max:4096*4096, min:16*16), leftOffset and upOffset cannot
    // larger than real width and height
    ...
    // restriction: leftOffset and upOffset of inputputArea must be even, rightOffset and
    // downOffset of inputputArea must be odd.
...
int CollisionPreProcess::__handle_vpc(const ImageData<u_int8_t>& img, int64_t timestamp) {
    if (NULL == img.data || NULL == img.data.get()) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionPreProcess]img.data is null, ERROR");
        return HIAI_ERROR;
    }

    if (IMAGE_TYPE_NV12 != (ImageTypeT)img.format) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionPreProcess]the format is not yuv, ERROR");
        return HIAI_ERROR;
    }
    ...
    if (0 != (img.width % VPC_OUT_WIDTH_STRIDE) || 0 != (img.height % VPC_OUT_HIGH_STRIDE)) {
        ...
        if (NULL == align_buffer) {
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                            "[CollisionPreProcess]HIAI_DVPP_DMalloc align_buffer is null, ERROR");
            return HIAI_ERROR;
        }
        for (int i = 0; i < img.height; i++) {
            ...
            if (0 != ret) {
                HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionPreProcess]memcpy_s error in copy Y");
        ...
        for (int i = 0; i < img.height / 2; i++) {
            ...
            if (0 != ret) {
                HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionPreProcess]memcpy_s error in copy UV");
                HIAI_DVPP_DFree(align_buffer);
                align_buffer = NULL;
                return HIAI_ERROR;
            }
    ...
    if (HIAI_OK != ret) {  // if vpc process with error, return directly, and then process next.
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                        "[CollisionPreProcess] call DVPP_CTL_VPC_PROC process faild");
...
int CollisionPreProcess::__handle_vpc_with_param(const unsigned char* buffer, const int& width,
    ...
    HIAI_ENGINE_LOG("[CollisionPreProcess] real_width: %d, real_height: %d, width: %d, height: %d",
    ...
    if (type == FILE_TYPE_PIC_JPEG) {
        switch (format) {  // format is responding to color sapce after decoder, which is needed to
            ...
            default:
                HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                                "[CollisionPreProcess]Jpegd out format[%d] is error", format);
                break;
        }
        ...
        switch (format) {
            ...
            default:
                HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                                "[CollisionPreProcess]Pngd out format[%d] is error", format);
                break;
        }
    } else {
    ...
    if (0 == resize_width || 0 == resize_height) {
        HIAI_ENGINE_LOG(HIAI_IDE_INFO,
                        "[CollisionPreProcess] user donnot need resize, resize width/height use "
                        "real size of pic");
    ...
    if (resize_width > DVPP_SUPPORT_MAX_WIDTH || resize_width < DVPP_SUPPORT_MIN_WIDTH ||
        ...
        HIAI_ENGINE_LOG(
            HIAI_IDE_ERROR,
            "[CollisionPreProcess]resize range error, resize_width:%u, resize_height:%u",
    ...
    if (__process_crop(inputConfigure->cropArea, width, height, real_width, real_height)) {
        ...
        if (nullptr == outputConfigure->addr) {
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionPreProcess]outputConfigure->addr is null");
            return HIAI_ERROR;
        }
        HIAI_ENGINE_LOG(HIAI_IDE_INFO,
                        "[CollisionPreProcess]inputConfigure cropArea:%u, %u, %u, %u, "
        ...
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionPreProcess]__process_crop error");
        return HIAI_ERROR;
    }
    ...
    if (!m_dvpp_config_->dvpp_para.empty()) {
        ...
        HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[CollisionPreProcess]dvpp_para:%s",
    ...
    if (0 != DvppCtl(m_pidvppapi_, DVPP_CTL_VPC_PROC, &dvppApiCtlMsg)) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionPreProcess] call dvppctl process:VPC faild");
    ...
    if (NULL == m_dvpp_out_) {
        ...
        if (NULL == m_dvpp_out_ || NULL == m_dvpp_out_.get()) {
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                            "[CollisionPreProcess] call m_dvpp_out_ make_shared failed");
            HIAI_DVPP_DFree(outputConfigure->addr);
            outputConfigure->addr = nullptr;
            return HIAI_ERROR;
        }
    ...
    if (NULL == image || NULL == image.get()) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionPreProcess] call image make_shared failed");
        HIAI_DVPP_DFree(outputConfigure->addr);
        outputConfigure->addr = nullptr;
        return HIAI_ERROR;
    }
    ...
    try {
        ...
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                        "[CollisionPreProcess] failed to allocate buffer for out_buffer");
        return HIAI_ERROR;
    }
    std::shared_ptr<u_int8_t> out_buffer(tmp);
    if (NULL == out_buffer || NULL == out_buffer.get()) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionPreProcess] call out_buffer shared_ptr failed");
        HIAI_DVPP_DFree(outputConfigure->addr);
        outputConfigure->addr = nullptr;
        return HIAI_ERROR;
    }
    ...
    outputConfigure->addr = nullptr;
    if (ret != 0) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionPreProcess] memcpy_s out buffer data error");
        out_buffer = nullptr;
        return HIAI_ERROR;
    }
    ...
    if (1 == m_dvpp_config_->dump_value) {
        HIAI_ENGINE_LOG(HIAI_IDE_INFO,
                        "[CollisionPreProcess]preprocess need dump, width:%u, height:%u",
...
int CollisionPreProcess::storePreprocessImage(const u_int8_t* out_buffer, const uint32_t& size,
                                              const DvppPreprocessInfo& info) {
    if (NULL == out_buffer) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                        "[CollisionPreProcess]storePreprocessImage, out_buffer is null!");
        return HIAI_ERROR;
    }
    ...
    std::string fileName = "/projects/" + m_dvpp_config_->project_name +
                           "/out/PreProcess/CollisionPreProcess/" +
                           std::to_string(m_imageFrameID) + "_" +
                           std::to_string(m_orderInFrame + 1) + "_preprocessYUV";
    ...
    if (NULL == session) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                        "[CollisionPreProcess]storePreprocessImage, ideOpenFile is null!");
        return HIAI_ERROR;
    }
    ...
    while (sendSize < size) {
        ...
        if (IDE_DAEMON_NONE_ERROR != ideWriteFile(session, tmp, everySize)) {
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                            "[CollisionPreProcess]storePreprocessImage, ideWriteFile error!");
    ...
    if (IDE_DAEMON_NONE_ERROR != ideCloseFile(session)) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                        "[CollisionPreProcess]storePreprocessImage, ideCloseFile error!");
        return HIAI_ERROR;
    }
    HIAI_ENGINE_LOG(
        HIAI_IDE_INFO,
        "[CollisionPreProcess]storePreprocessImage, storage success!, sendSize:%u, size:%u",
        sendSize, size);
    return HIAI_OK;
}

int CollisionPreProcess::__handle_dvpp() {
    if (NULL == m_dvpp_in_ || NULL == m_dvpp_in_.get()) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionPreProcess]__hanlde_dvpp, input data is null!");
        return HIAI_ERROR;
    }
            ...
            if (NULL != m_dvpp_crop_in_ && NULL != m_dvpp_crop_in_.get()) {
                ...
                for (std::vector<Rectangle<Point2D>>::iterator iterRect =
                    ...
                    if (HIAI_OK != __update_crop_para(*iterRect) ||
                        ...
                        HIAI_ENGINE_LOG(
                            HIAI_IDE_WARNING,
                            "[CollisionPreProcess]Handle One Image Error, Continue Next");
                        continue;
                    }
                }
            } else {
...
/**
* @ingroup hiaiengine
* @brief HIAI_DEFINE_PROCESS: realize CollisionPreProcess
* @[in]: define a input and output,
*        and register Engine named CollisionPreProcess
*/
HIAI_IMPL_ENGINE_PROCESS("CollisionPreProcess", CollisionPreProcess, INPUT_SIZE) {
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[CollisionPreProcess] start process!");
    ...
    if (arg0 != NULL) {
        ...
        if (!isSentinelImage(dataInput)) {
            ...
            if (m_data_input_in_->v_img.size() != m_data_input_in_->b_info.batch_size) {
                HIAI_ENGINE_LOG(
                    HIAI_IDE_INFO, "[CollisionPreProcess] Wait for other %d batch image info!",
    ...
    if (!input_que_.PopAllData(m_dvpp_in_, m_dvpp_crop_in_)) {
        HIAI_ENGINE_LOG(HIAI_IDE_WARNING, "[CollisionPreProcess]fail to pop all data");
        return HIAI_ERROR;
    }
#else
    ...
    if (isSentinelImage(m_dvpp_in_)) {
        HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[CollisionPreProcess]sentinel Image, process over.");
        HIAI_StatusT hiai_ret = HIAI_OK;
        do {
            hiai_ret = SendData(0, "BatchImageParaWithScaleT",
                                std::static_pointer_cast<void>(m_dvpp_in_));
            if (HIAI_OK != hiai_ret) {
                if (HIAI_ENGINE_NULL_POINTER == hiai_ret || HIAI_HDC_SEND_MSG_ERROR == hiai_ret ||
                    ...
                    HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                                    "[CollisionPreProcess] SendData error[%d], break.", hiai_ret);
                    break;
                }
                HIAI_ENGINE_LOG(
                    HIAI_IDE_INFO,
                    "[CollisionPreProcess] SendData return value[%d] not OK, sleep 200ms",
                    hiai_ret);
                usleep(SEND_DATA_INTERVAL_MS);
            }
    ...
    if (HIAI_ERROR == err_code) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionPreProcess]dvpp process error!");
        goto ERROR;
    }
    ...
    __clear_data();
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[CollisionPreProcess] end process!");
    return HIAI_OK;
