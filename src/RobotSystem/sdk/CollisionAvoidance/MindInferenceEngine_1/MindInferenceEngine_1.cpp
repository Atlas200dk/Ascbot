...
*/
void CollisionInferenceEngine::ClearOutData() {
...
*/
HIAI_StatusT CollisionInferenceEngine::Init(
    const hiai::AIConfig& config, const std::vector<hiai::AIModelDescription>& model_desc) {
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[CollisionInferenceEngine] start init!");
        ...
        if (item.name() == "model_path") {
            ...
            if (model_path.empty()) {
                HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                                "[CollisionInferenceEngine] model_path not exist!");
                return HIAI_ERROR;
            }
            ...
            ss >> batch_size;
            if (batch_size <= 0) {
                HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionInferenceEngine] batch_size <= 0!");
                return HIAI_ERROR;
            }
        }
    }
    ...
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[CollisionInferenceEngine] end init!");
    return HIAI_OK;
}
...
*/
void CollisionInferenceEngine::HandleExceptions(std::string error_msg) {
    ...
    if (ret != HIAI_OK) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionInferenceEngine] send data failed!");
    }
}
...
*/
HIAI_StatusT CollisionInferenceEngine::SendSentinelImage() {
    ...
    do {
        HIAI_ENGINE_LOG(HIAI_IDE_INFO,
                        "[CollisionInferenceEngine] sentinel image, process success!");
        ...
        if (HIAI_OK != hiai_ret) {
            if (HIAI_ENGINE_NULL_POINTER == hiai_ret || HIAI_HDC_SEND_MSG_ERROR == hiai_ret ||
                ...
                HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                                "[CollisionInferenceEngine] SendData error[%d], break.", hiai_ret);
                break;
            }
            HIAI_ENGINE_LOG(
                HIAI_IDE_INFO,
                "[CollisionInferenceEngine] SendData return value[%d] not OK, sleep 200ms",
                hiai_ret);
...
* @return: HIAI_StatusT
*/
HIAI_StatusT CollisionInferenceEngine::PrepareInputBuffer(uint8_t* input_buffer,
    ...
    if (input_buffer == nullptr) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                        "[CollisionInferenceEngine] ERROR, input_buffer is nullptr");
        return HIAI_ERROR;
    }
    for (int j = 0; j < batch_size; j++) {
        if (batch_begin + j < image_number) {
            if (memcpy_s(input_buffer + j * image_size, image_size,
                ...
                HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                                "[CollisionInferenceEngine] ERROR, copy image buffer failed");
                return HIAI_ERROR;
            }
        } else {
            if (memset_s(input_buffer + j * image_size, image_size, static_cast<char>(0),
                ...
                HIAI_ENGINE_LOG(
                    HIAI_IDE_ERROR,
                    "[CollisionInferenceEngine] ERROR, batch padding for image data failed");
                return HIAI_ERROR;
            }
        }
    }
...
* @return: HIAI_StatusT
*/
HIAI_StatusT CollisionInferenceEngine::PrepareInforInput(
    ...
    for (int j = 0; j < batch_size; j++) {
        if (batch_begin + j < image_number) {
            ...
            HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[CollisionInferenceEngine] info each input size: %d",
                            each_size);
            if (memcpy_s(input_buffer2 + j * each_size, each_size, _input_arg_2.data.get(),
                         each_size)) {
                HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                                "[CollisionInferenceEngine] ERROR, copy info buffer failed");
                return HIAI_ERROR;
            }
        } else {
            float info_tmp[3] = {0.0, 0.0, 0.0};
            each_size = sizeof(info_tmp);
            HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[CollisionInferenceEngine] info padding size: %d",
                            each_size);
            if (memcpy_s(input_buffer2 + j * each_size, each_size, info_tmp, each_size)) {
                HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                                "[CollisionInferenceEngine] ERROR, padding info buffer failed");
                return HIAI_ERROR;
            }
        }
    }
...
* @return: HIAI_StatusT
*/
HIAI_StatusT CollisionInferenceEngine::Predict() {
    ...
    for (uint32_t index = 0; index < outputTensorVec.size(); index++) {
        ...
        if (hiai_ret != HIAI_OK || buf == nullptr) {
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionInferenceEngine] HIAI_DMalloc failed.");
            ClearOutData();
            return HIAI_ERROR;
        }
    ...
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[CollisionInferenceEngine] ai_model_manager_->Process start!");
    ret = ai_model_manager_->Process(ai_context, input_data_vec, output_data_vec, 0);
    if (hiai::SUCCESS != ret) {
        ClearOutData();
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                        "[CollisionInferenceEngine] ai_model_manager Process failed");
        return HIAI_ERROR;
    }
    return HIAI_OK;
}
...
* @return: HIAI_StatusT
*/
HIAI_StatusT CollisionInferenceEngine::SetOutputStruct(const int batch_begin) {
    for (int i = 0; i < output_data_vec.size(); ++i) {
        HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[CollisionInferenceEngine] build: %d", i);
        ...
        HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[CollisionInferenceEngine] build: %d, number: %d",
...
*/
void CollisionInferenceEngine::SendResult() {
    HIAI_StatusT hiai_ret = HIAI_OK;
    do {
        hiai_ret = SendData(0, "EngineTransT", std::static_pointer_cast<void>(tran_data));
        if (HIAI_QUEUE_FULL == hiai_ret) {
            HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[CollisionInferenceEngine] queue full, sleep 200ms");
            usleep(200000);
        }
    } while (hiai_ret == HIAI_QUEUE_FULL);
    if (HIAI_OK != hiai_ret) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                        "[CollisionInferenceEngine] SendData failed! error code: %d", hiai_ret);
    }
}
...
* @in: index of the begin of this batch
*/
void CollisionInferenceEngine::HandleModelBatchFailure(const int batch_begin,
                                                       const int image_number) {
...
*/
HIAI_IMPL_ENGINE_PROCESS("CollisionInferenceEngine", CollisionInferenceEngine, INPUT_SIZE) {
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[CollisionInferenceEngine] start process!");
    hiai::AIStatus ret = hiai::SUCCESS;
    HIAI_StatusT hiai_ret = HIAI_OK;
    ...
    if (nullptr != arg0) {
        ...
        if (!isSentinelImage(dataInput)) {
            ...
            if (m_data_input_in_->v_img.size() != m_data_input_in_->b_info.batch_size) {
                HIAI_ENGINE_LOG(
                    HIAI_IDE_INFO,
                    "[CollisionInferenceEngine] Wait for other %d batch image info!",
    ...
    if (!input_que_.PopAllData(image_handle)) {
        HandleExceptions("[CollisionInferenceEngine] fail to PopAllData");
        return HIAI_ERROR;
    }
#endif
    ...
    if (nullptr == image_handle) {
        HandleExceptions("[CollisionInferenceEngine] Image_handle is nullptr");
        return HIAI_ERROR;
    }
    ...
    if (nullptr == _multi_input_2) {
        HandleExceptions("[CollisionInferenceEngine] fail to process invalid message");
        return HIAI_ERROR;
    }
    int info_number = _multi_input_2->v_info.size();
    if (info_number != image_number) {
        HandleExceptions(
            "[CollisionInferenceEngine] ERROR the number of image data and information data "
            "doesn't match!");
    }
    ...
    if (hiai_ret != HIAI_OK || _input_buffer2 == nullptr) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                        "[CollisionInferenceEngine] HIAI_DMalloc _input_buffer2 failed.");
        return HIAI_ERROR;
    }
#endif
    ...
    if (_input_buffer1_size <= 0) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionInferenceEngine] _input_buffer1_size <= 0");
        return HIAI_ERROR;
    }
    ...
    if (hiai_ret != HIAI_OK || _input_buffer1 == nullptr) {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR,
                        "[CollisionInferenceEngine] HIAI_DMalloc _input_buffer1 failed.");
#if (INPUT_SIZE == 3)
    ...
    for (int i = 0; i < image_number; i += batch_size) {
        ...
        if (HIAI_OK != PrepareInforInput(_input_buffer2, image_number, i, _multi_input_2)) {
            // HandleExceptions("[CollisionInferenceEngine] batch " +
        ...
        if (tran_data->output_data_vec.empty()) {
            ...
            HIAI_ENGINE_LOG(HIAI_IDE_INFO,
                            "[CollisionInferenceEngine] alloc memory for dataset batch, number of "
            ...
            for (int i = 0; i < output_data_vec.size(); i++) {
                ...
                if (out.size <= 0) {
                    HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[CollisionInferenceEngine] out.size <= 0");
                ...
                HIAI_ENGINE_LOG(
                    HIAI_IDE_INFO,
                    "[CollisionInferenceEngine] cnt_model_batch: %d, image number: %d!", cnt_batch,
                    image_number);
            }
        }
    ...
    ClearOutData();
    tran_data = nullptr;
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[CollisionInferenceEngine] end process!");
    return HIAI_OK;
}
